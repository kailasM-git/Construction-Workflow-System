...
function ConstructorTable() {
  ...
  const handleSubmit = (id) => {
    ...
    fetch('http://localhost:4000/consultant/approveconstructor', {
      method: "post",
      headers: {
  ...
  const handleSubmitrej = (id) => {
    ...
    fetch('http://localhost:4000/consultant/rejectconstructor', {
      method: "post",
      headers: {
        Accept: "application/json",
        'Content-Type': "application/json"
    ...
    <div style={{
      ...
      <div style={{ overflowX: 'auto' }}>
        <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: '800px' }}>
          <thead>
            <tr style={{ backgroundColor: '#f1f5f9', textAlign: 'left' }}>
              {['#', 'Name', 'Address', 'License', 'Experience', 'Email', 'Actions'].map((header, i) => (
                <th key={i} style={{ padding: '14px 20px', color: '#475569', fontSize: '13px', textTransform: 'uppercase', fontWeight: 600 }}>
                  {header}
                </th>
              ))}
            </tr>
          ...
          {/* Instead of tbody, direct mapping here */}
          {view.map((items, index) => (
            ...
            <tr key={index} style={{ borderTop: '1px solid #e2e8f0', transition: '0.2s' }}>
              ...
              <td style={{ padding: '14px 20px' }}>
                ...
                ): items.regtype === "consultcontractorregister" ? (
